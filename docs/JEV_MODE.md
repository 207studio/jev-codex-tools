...
Give each child a narrow task, explicit policies and `jev: per-step`; require actual Jev calls for semantic choices. Inspect `jev-mode coverage --task <session-id>` after completion. Its v1.2.0 parser recognizes `codex-typesafe` call strings and a separate hook trace, so it does not establish coverage of every local Jev adapter. Missing trace or unmatched tool names mean unmeasured coverage. Historical reports of child hook gaps are not proof of the current host's behavior.
