...
With trusted hooks and `visual_enforcement` enabled, recognized visual calls must complete a finite routing consultation. These include image-view/screenshot/image-generation tools, recognized Figma tools, CUA code with explicit capture calls, supported UI-file edits, and detectable screenshot shell commands already subject to the shell guard. Indirect hosted scripts, unknown nested execution, and tools that the host does not expose to hooks are not universally intercepted. Exact registered helpers are exempt from nested routing decisions.
